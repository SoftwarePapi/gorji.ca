...
import {
  ...
  dictionary,
  matchers,
} from "__GENERATED__/client-manifest.js";
...
function create_client({ target, session, base, trailing_slash }) {
  ...
  async function update(url, redirect_chain, no_cache, opts, callback) {
    ...
    if (opts) {
      ...
      if (autoscroll) {
        const deep_linked =
          url.hash && document.getElementById(url.hash.slice(1));
        if (scroll) {
          scrollTo(scroll.x, scroll.y);
        } else if (deep_linked) {
  ...
  async function get_navigation_result_from_branch({
    ...
    const filtered = /** @type {import('./types').BranchNode[] } */ (
    ...
    for (const route of routes) {
      ...
      if (params) {
        const normalized = new URL(
          url.origin +
            normalize_path(url.pathname, trailing_slash) +
            url.search +
            url.hash
        );
  ...
  return {
    ...
    prefetch_routes: async (pathnames) => {
      const matching = pathnames
        ? routes.filter((route) =>
            pathnames.some((pathname) => route.exec(pathname))
          )
        : routes;

      const promises = matching.map((r) =>
        Promise.all(r.a.map((load) => load()))
      );
